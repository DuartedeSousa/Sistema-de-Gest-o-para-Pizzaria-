...
async function fazerLogin() { // Função resposável por fazer o login e verificar a validade do mesmo
...
function sair() { // Função que permite o usuário logado sair da sessão iniciada
  TOKEN = '';
  USUARIO_LOGADO = null;
...
function toast(msg, tipo = 'ok') { // Define uma mensagem com cores no fundo (podendo ser verde ou vermelho por conta do css)
...
function abrir(id)  { document.getElementById(id).classList.add('open'); } // Função que permite visualizar o id
function fechar(id) { document.getElementById(id).classList.remove('open'); } // Função que anula a visualização

document.querySelectorAll('.modal-bg').forEach(bg =>
  bg.addEventListener('click', e => { if (e.target === bg) bg.classList.remove('open'); })
);

function R$(v) { // Função responsável por regionalizar (trocando o "." pela vírgula, ademais se apresentando com "R$" ao invés de outros sistemas monetários) e apresentar os valores da pizza
  return 'R$ ' + Number(v || 0).toFixed(2).replace('.', ',');
}

function badge(s) { // Função resposável por definir a "badge" dos status dos pedidos
  const r = {
    recebido:     '📥 Recebido',
...
async function api(method, url, body) { // Consulta da API
  const opts = {
    method,
    headers: {
...
function aplicarPerfil(usuario) { // Aplica nos tipos diferentes de perfis diferentes tipos de visibilidade do sistema (permissões para ver dados, utilizar certas funcionalidades, etc.)
  ...
  function show(id, visible, type = 'flex') { // Verifica se algo estpa visível ou não
    const el = document.getElementById(id);
    if (el) el.style.display = visible ? type : 'none';
  }

  function showEl(el, visible, type = 'flex') { // Verifica se o "el" está visível ou não
    if (el) el.style.display = visible ? type : 'none';
  }
...
async function carregarMesas(mesaFiltro = null) { // Reponsável por carregar as mesas arquivadas na base de dados para a visualição
...
async function abrirPedidoMesa(mesaNum = null) { // Permite a criação de um pedido para uma mesa específica
  try {
...
function addItemMesa() { // Permite adicionar um novo item a uma mesa
...
async function salvarPedidoMesa() { // Responsável por salvar os pedidos de cara mesa, incluindo preço dos itens, observações, etc.
...
function abrirFecharMesa(mesa, total, ids) { // Define o fechamento ou abertura de uma mesa, e se seus pedidos foram entregues à mesma, informando também o total a pagar
...
async function confirmarFechamento() { // Confirma o fechamento de uma mesa
...
async function carregarDashboard() { // Responsável por carregar o dashboard
...
async function carregarPizzas() { // Reponsável por carregar as pizzas arquivadas na base de dados para a visualição 
...
function abrirPizza() { // Abre a possibilidade da criação de uma nova pizza
...
function editarPizza(id) { // Responsável por permitir a edição de uma pizza já existente
...
async function salvarPizza() { // Salva uma nova pizza ou uma pizza editada no banco de dados
...
async function deletarPizza(id, nome) { // Permite deletar uma pizza
...
function abrirCliente() { // Responsável por criar um novo cliente
...
function editarCliente(id) { // Edita um cliente por meio do id
...
async function salvarCliente() { // Salva um novo cliente ou um cliente editado
...
async function deletarCliente(id, nome) { // Deleta cliente já existente
  if (!confirm(`Deletar "${nome}"?`)) return;
  try {
...
async function abrirPedido() { // Criação de um pedido
  try {
    if (!cPizzas.length)   cPizzas   = await api('GET', '/pizzas');
    if (!cClientes.length) cClientes = await api('GET', '/clientes');
  } catch (e) { toast('Erro ao carregar dados', 'err'); return; }
...
function addItem() { // Função para adicionar um item ao pedido
  const d = document.createElement('div');
  d.className = 'item-row';
  const opts = cPizzas
...
function abrirStatus(id, status) { // Permite edição de um status de um pedido
  document.getElementById('st-id').value  = id;
  document.getElementById('st-val').value = status;
  abrir('m-status');
}

async function salvarStatus() { // Salva os status editados acima
...
async function deletarPedido(id) { // Permite deletar um pedido
...
async function carregarUsuarios() { // Responsável por carregar uma lista de usuários com suas informações
...
function abrirUsuario() { // Cria um novo usuário
  ['u-nome','u-email','u-senha'].forEach(id => document.getElementById(id).value = '');
  document.getElementById('u-perfil').value = 'Atendente';
  abrir('m-usuario');
}

async function salvarUsuario() { // Salva os dados do novo usuário caso o mesmo tenha preenchido todos os campos
...
async function deletarUsuario(id, nome) { // Deleta um usuário existente
  if (!confirm(`Deletar "${nome}"?`)) return;
  try {
